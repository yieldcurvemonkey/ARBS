...
def build() -> dict:
    ...
    vols = vol_panel("USD-SOFR-1D", ["2y 10y"], FULL_START, END)
    ...
    raw_tfp = build_tfp_history(TWO_FACTOR_START, END)
...
def _adf_report(resid: pd.Series, label: str) -> None:
...
def _run_and_report(label: str, spread: pd.Series, drivers: dict) -> None:
    ...
    _adf_report(lev.residuals, label)
...
def main() -> None:
    ...
    print("root does not by itself establish that; see each ADF block's half-life above.")
