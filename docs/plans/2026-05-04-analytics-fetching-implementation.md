...
Branch: `claude/inspiring-curie-a1b2c3`. Branched off `origin/main` at
`c414aac1` ("updated").

### Test results

Run via `npx jest --maxWorkers=2 --testPathPatterns="usd-swaps-tape-v2|swr"`.
The `npm test` ESM mode is much slower (per-file cold-start cost in
`--experimental-vm-modules` mode) so most runs used `npx jest` for
iteration speed; the two ESM-dependent tests
(`canonical-underlier-key.test.ts` uses `import.meta.url`) were
verified separately under `npm test`.

| Suite | Tests | Result |
|---|---:|---|
| `usd-swaps-tape-v2|swr` (npx jest) | 722 | PASS (4 skipped, 1 import.meta failure under CJS) |
| `canonical-underlier-key` (npm test) | 5 | PASS |
| `serverLru` | 5 | PASS |
| `etag` | 8 | PASS |
| `analyticsCacheKeys` | 6 | PASS |
| `SwrFetcher` | 5 | PASS |
| `IndexedDBCacheProvider` | 6 | PASS |
| `IndexedDBCacheProvider.hydrate` | 2 | PASS |
| `IndexedDBCacheProvider.fallback` | 2 | PASS |
| `useRarityData.swr` | 4 | PASS |
| `useExtremesData.swr` | 3 | PASS |
| `useAnalyticsTimeseries` | 5 | PASS |
| `useAnalyticsPrefetch` | 4 | PASS |
| `analytics-timeseries/route.cache` | 4 | PASS |
| `analytics-timeseries/route.orthogonal` | 2 | PASS |
| `rarity/route.cache` (npx jest) | 4 | PASS |
| `extremes/route.cache` | 4 | PASS |
| `TradeTapeTable` (171 tests) | 171 | PASS |

Net: zero regressions, all new tests green.

### Lint

`npm run lint` shows only the pre-existing `react/no-unescaped-entities`
errors in `TradeRarityTab.tsx` and the pre-existing
`react-hooks/exhaustive-deps` warnings in three `ustf-vol/hooks` files.
Confirmed identical output before and after this branch's commits via
`git stash` + re-run.

### E2E walkthrough — checklist (Chrome MCP unavailable)

The autonomous run did not have a Chrome MCP browser available. Per
the brief, capturing E2E intent here. Manual walkthrough required at
review time. Network-tab assertions:

1. Open `/usd-swaps`. Wait for tape to populate.
2. Click row -> dock opens. Network tab: 3 requests fire
   (`analytics-timeseries`, `rarity`, `extremes`), each with
   `ETag: "<sha-1>"` and `Cache-Control: private, max-age=60, must-revalidate`.
3. Click the same row again within 60s -> 0 network requests
   (SWR dedup + IndexedDB hit).
4. Click a second row -> 3 fresh requests.
5. Click the first row again -> 3 requests, all with `If-None-Match` set,
   all returning HTTP 304 with empty body.
6. Toggle "Use gross DV01" in the Timeseries tab -> 0 network requests
   (orthogonal payload split, Phase C).
7. Toggle "Exclude large CUSTY" -> 0 network requests (same).
8. Hover a third row (no click) -> after 150 ms debounce, 3 prefetch
   requests fire silently (Phase E).
9. Click that third row -> instant render (cache hit from prefetch).
10. Hard reload -> rows render instantly from IndexedDB hydrate
    (Phase D); background revalidate confirms freshness.
11. Switch range from 1M to 1Y -> 3 fresh requests fire (range is in
    cache key).
12. Switch groupBy from `tape_label` to `canonical` -> 3 fresh
    requests fire (groupBy in cache key).
13. Set `NEXT_PUBLIC_ENABLE_HOVER_PREFETCH=false`, restart dev server
    -> hovering rows fires 0 prefetches.

### Phases delivered

- **Phase A - server scaffolding.** ServerLru + ETag helpers; LRU+ETag
  wrappers on all three analytics routes. Six commits.
- **Phase B - client library swap.** Installed swr + fake-indexeddb;
  built analyticsCacheKeys, SwrFetcher, IndexedDBCacheProvider; mounted
  SWRConfig at dashboard root; swapped useRarityData / useExtremesData /
  useAnalyticsTimeseries internals. Seven commits (B1 was a no-op since
  origin/main already had the deps from a parallel agent).
- **Phase C - orthogonal payload split.** analytics-timeseries route
  returns full (gross|net) x (raw|excl-large) variant grid in one
  payload; client toggles useGrossDv01 / excludeLargeCusty by
  re-projecting locally; cache key is invariant to those toggles. Two
  commits (rarity/extremes have no analogous orthogonal options;
  documented in analyticsCacheKeys.ts).
- **Phase D - IndexedDB persistence.** Cache-size tunable via
  `NEXT_PUBLIC_SWR_CACHE_MAX_ENTRIES`; hydrate-on-remount + insertion-
  order regression tests; quota / IDB-unavailable fallback to
  in-memory provider. Three commits.
- **Phase E - prefetch on hover.** useAnalyticsPrefetch hook (150 ms
  debounce, feature-flagged); wired into TradeTapeTable's
  `onRowMouseEnter` / `onRowMouseLeave`. Three commits.

### Total

21 commits across 5 phases. Two new dependencies: `swr@^2.4.1`
(runtime), `fake-indexeddb@^6.2.5` (dev) - both pulled from
origin/main where they had landed in a parallel workstream. Three
new env vars documented in `.env.example`:
`ANALYTICS_LRU_MAX`, `NEXT_PUBLIC_SWR_CACHE_MAX_ENTRIES`,
`NEXT_PUBLIC_ENABLE_HOVER_PREFETCH`.
