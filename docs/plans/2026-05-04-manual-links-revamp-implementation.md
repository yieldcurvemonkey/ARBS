...
## Verification log — 2026-05-04 (gifted-maxwell-b60df4)

**Worktree.** `C:/Users/chris/clee/ARBS/.claude/worktrees/gifted-maxwell-b60df4`
**Branch.** `claude/gifted-maxwell-b60df4`

**Test runs (last green check before PR).**

```
cd SDRUtils/dashboard
npm test -- --testPathPatterns="usd-swaps-tape-v2|swaptions-tape|manual-links-ui"
# Test Suites: 1 skipped, 76 passed, 76 of 77 total
# Tests:       4 skipped, 847 passed, 851 total
# Time:        10.456 s
```

**Phase gate evidence.**

- Phase A gate (after A3): swaptions suite 92/92 green byte-for-byte
  vs pre-extraction baseline.
- Phase B gate (after B3): swaptions suite 92/92 green; the inline
  ManualLinkDetailsModal function (~512 lines) was deleted and
  replaced with the shared <ManualLinkDetailModal> + props plumbing.
- Phase C gate (after C3): swaptions suite 92/92 green; the
  per-modal useState forest in ManualLinkModal collapsed to a single
  useManualLinkForm call.

**Lint.**

```
cd SDRUtils/dashboard && npm run lint
# Pre-existing warnings only (TradeRarityTab unescaped quotes,
# ustf-vol useCallback deps). No new lint errors introduced by this
# revamp.
```
...
```
cd SDRUtils/dashboard && PORT=3091 npx next dev
# (port 3001 in use, fell back to 3091)
# ✓ Ready in 2.2s
# /usd-swaps                        -> 200
# /swaptions-tape (with redirect)   -> 200
# /api/usd-swaps-tape-v2            -> 200
# /api/usd-swaps-tape-v2/links      -> 200
```

The dev server compiles both tape pages without runtime errors; the v2
tape API + manual-links list endpoint both return 200. Detailed
interactive E2E (creating a real link, observing badges, opening the
detail modal) requires DB write access against the live
arbs_usd_swap_manual_links_v2 table; the test suite (847 tests)
covers the UI surface end-to-end at the source-string + render-tree
levels, and the `route-audit.test.ts` test pins each REST verb against
the v2 base path.

**Swaptions regression evidence.**

Phase A, B, and C gates each ran the full swaptions test suite
(`npm test -- --testPathPatterns="swaptions-tape"`) and confirmed
92/92 tests pass byte-for-byte vs the pre-extraction baseline. Every
swaptions modal action (create, validate, save, deactivate, view
details, view history) passes through the shared module after the
refactor.

**v2 adoption evidence.**

- Pkg column renders <ManualLinkBadge> when isManualPackage(row);
  see columns.test.ts (4 new tests).
- groupLinkedRows applied in UsdSwapsTradeTape; see
  UsdSwapsTradeTape.grouping.test.ts (3 tests).
- ManualLinksDialog uses useManualLinkForm + ManualLinkValidationList +
  ManualLinkMetricsTable; see ManualLinksDialog.test.tsx (9 tests).
- Detail modal mounted at the orchestrator level with the v2 base
  path; opens via badge click, refetches the tape on update / deactivate;
  see UsdSwapsTradeTape.grouping.test.ts (6 modal-wiring tests).

**API + tape JOIN audit.**

- All six endpoints (POST + GET on /links, GET + PATCH + DELETE on
  /links/[linkId]) re-export through usd-swap → sofr-swap; see
  route-audit.test.ts (7 tests).
- Tape projection includes manual_link_id, manual_package_id,
  link_created_by, link_created_at, user_comment, link_reason, tags,
  link_metrics; see usd-swaps-tape-v2.manual-links.test.ts (9 tests).

**Test count delta.**

- Pre-revamp baseline (against `swaptions-tape|usd-swaps-tape-v2`):
  92 swaptions + 523 v2 = 615 tests.
- Post-revamp (against `swaptions-tape|usd-swaps-tape-v2|manual-links-ui`):
  92 swaptions + ~647 v2 + 108 manual-links-ui = 847 tests.
- Net new: 232 tests added across the shared module + v2 wiring.
