...
Every finding above has been closed. The remediation ran as a 19-task
...
- **`dealer_spread_bps` is now true basis points** (`residual /
  total_dv01`, the ×100 dropped). Verified on live Supabase after
...
- **`ptp_price_notation` persisted end-to-end** (SMALLINT on the
  ...
  234 / 17.
- **Null-OPA legs keep `opa_sign = NULL`** (a sign has no economic
  meaning without an amount). Post-backfill count of signed-null-OPA
  rows: 0 on both dates.
- **Dashboard column list is now auto-projected** from the DB view via
  a 14-entry exclusion list (`EXCLUDED_VIEW_COLUMNS`) — new view
  columns appear without a hand-edit. **NUMERIC/BIGINT parsing moved to
  global pg type parsers** (`types.setTypeParser`), replacing the
  per-route coercion loop.
- **Sub-fly annotation hardened**: distinct-tenor precheck
  (`round(1)`), deterministic (pv01, rate) ordering, `fixed_rate`
  threaded into classification.
- **Incremental-hood integrity**: the 5-minute hood now expands to
  whole PTP-key groups (±5 s) before detection, so a group straddling
  the hood boundary is never split.
- **`df.get` Series-default guards** on the four package columns
  (scalar defaults broke boolean indexing on older frames).
- **Caches re-keyed**: `DETECTION_CACHE_VERSION = "ptp2"`,
  `TRADE_TAPE_CACHE_VERSION = "v10-ptp-truebp-notation"`; both audit
  dates re-backfilled on the new code and re-verified in the DB and in
  Chrome (0.0003 bp display, PX chip on notation-3 PKG-8). Tape service
  restarted on the fixed tree.

### Phase B — test estate to green (Tasks 12–19)

- Collection restored: the 7 orphaned test files deleted; **2442 tests
  collect with 0 errors**.
- Markers `slow` / `network` / `db` registered and applied; the fast
  developer gate is `-m "not slow and not network and not db"`
  (documented in root `CLAUDE.md`).
- The ~70 pre-existing failures were triaged test-by-test under a
  strict rule — production code changes only with git-archaeology
  proof; otherwise revert + skip with a ticket reason. Unproven
  production edits smuggled in by two subagents were caught in review
  and reverted (`aee5906d`, `257db6c9`).
- The order-dependent `assignment destination is read-only` failures
  were root-caused (commit `351227db`): `pandas_ta/core.py:19` sets
  `pd.options.mode.copy_on_write = True` at **import** time, and two
  backtest test files import it transitively during pytest
  *collection*; under CoW, `mms.py:332`'s in-place `m &= …` then hits a
  read-only buffer. Fixed with an autouse conftest fixture resetting
  CoW before every test. `tests/test_ustf_basis_pricer.py` (live
  Barchart fixtures) marked `slow + network`.
- One last flake surfaced by the fast-gate ordering: the duckdb
  bulk-upsert test's uncalibrated 2.0 s wall-clock bound pays cold
  first-touch cost when the slow tests are deselected (measured 2.16 s
  idle). Relaxed to 10 s — still catches the per-row-commit pathology
  it guards against (`b57b98f4`).

### Final suite state (commit `b57b98f4`)

| Gate | Result |
| --- | --- |
| Full pytest gate `-m "not network and not db"` | **2378 passed**, 43 skipped, 16 deselected, 2 xfailed, 3 xpassed, **0 failed, 0 errors** (59:54, at `351227db`) |
| Fast gate `-m "not slow and not network and not db"` | **2353 passed**, 43 skipped, 46 deselected, **0 failed** (1:59) |
| Dashboard `tsc --noEmit` | exit 0 |
| Dashboard jest (`npm test`) | **1279 passed**, 7 skipped, **0 failed** (12.8 s) |

Note: dashboard jest must run via `npm test` (which passes
`--experimental-vm-modules`); a plain `npx jest` bypasses the
`jest.unstable_mockModule` ESM mocks and the route.cache suites hit the
real Supabase DB (≈8 false failures with 60–100 s timeouts).

### Open items for the owner (tickets, not defects)

- Three swaption tests skip+ticket because they assert features that
  were never implemented: straddle platform allow/blocklists,
  in-function `link_packages` for the swaptions tape (needs
  `DETECTION_CACHE_VERSION` + `TRADE_TAPE_CACHE_VERSION` bumps if ever
  enabled), and `min_legs` enforcement.
- 12 STIRFO tests skip pending owner triage; 2 spread-MDP tests skip on
  a structural incompatibility (`IRSwapSpreadPricer` lacks
  `pricer_a/pricer_b` required by `SpreadProductAdapter`).
- `mms.py:332` would still crash in any process that imports
  `pandas_ta` (directly or via `BT.signals`) and then runs the MMS
  detector — no production code does today. Defensive candidate:
  `m = m & (…)` (behavior-identical).
- Price-notation mega-groups (e.g. PKG-107-style) are kept and tagged
  "PX" per the owner decision; revisit if they prove noisy.

*Addendum written by the 2026-07-02 remediation session.*
