...
```
python -m SDRUtils._swappulse_scripts.run_usdswaps_pipeline backfill --date YYYY-MM-DD
```
...
- **v2 immutability proof.** Snapshot `max(created_at)` and row count on both v2 tables before
  starting; assert unchanged at the end. This is the evidence that `sky` was never touched —
  the absence of an error is not that evidence.
...
1. v3's distinct `as_of_date` set equals v2's — 611 days.
...
4. `arbs_usd_swap_tape_legs_v2` and `_packages_v2` are unchanged in row count and
   `max(created_at)`.
...
| Backfill accidentally targets v2 | Writer guard, plus the before/after immutability proof |
