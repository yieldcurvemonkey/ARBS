...
**CONFIRMED** — A FIFO queue model is not uniformly correct across this CME product set; the answer is three-way: (1) ZN/ZF/ZB outrights plausibly FIFO (TN/UB unconfirmed); (2) ZT outrights NOT FIFO, grouped with ZQ and the ags under Configurable (K); (3) SR3 outrights NOT FIFO, Allocation (A): top-order percent, then pro-rata with a two-lot minimum, then FIFO, so size not queue position drives the pro-rata fill; (4) Treasury calendar spreads and 2Y/3Y butterfly spreads use K with pro-rata allocation.

Confirmed against CME primary sources, not the Databento blog. (P1) CME's own Globex Product Reference Sheet (.xls authored by CME, internal date cell 2026-02-13; retrieved via web.archive.org snapshot 20260214203145 of cmegroup.com/globex/files/globex-product-reference-sheet.xls) carries FIX tag 1142 plus the allocation parameters per product. OUTRIGHTS: ZN, ZF, ZB, TN, UB, Z3N, ES, NQ, CL = F (FIFO% 1.0); ZT = K with FIFO 40%/pro-rata 60%, pro-rata min 1, residual Leveling; ZQ = K, pro-rata 100%; SR1 = K; ZC/ZS/ZW = K (40/60); SR3 = A, top-order allocation on, pro-rata 100%, PRO-RATA MINIMUM ALLOCATION = 2, residual FIFO. That is exactly the claim's three-way split, including the ZT+ZQ+ags K grouping and the SR3 two-lot minimum. SPREADS: ZT-ZT/Z3N-Z3N/ZF-ZF/ZN-ZN/TN-TN = K 20% FIFO/80% pro-rata; ZB-ZB/UB-UB = K 100% pro-rata; ZT:BF and Z3N:BF = K 40/60 — while ZF:BF, ZN:BF, TN:BF, ZB:BF are plain F, so the claim's restriction of the pro-rata butterfly finding to 2Y/3Y only is precisely right. (P2) CME SER-8867 (29 Sep 2021) independently publishes the same percentages (calendars 80%, ZB/UB 100%, 2Y/3Y flies 60%, residual amended Leveling->FIFO); my parse of P1 reproduced all four figures, which validates the column mapping (checker run against a known answer first). (P3) CME's "Understanding SOFR Futures" states verbatim for SR3: "Allocation (A Algorithm, with Top Order Allocation = 100% and Pro Rata Allocation = 100%)", and for SR1 "Split FIFO and Pro-Rata (K Algorithm...)" — this killed the one live refutation candidate, a search summary reporting SR3 as "K", which had conflated SR1 with SR3. (P4) CME's Client Systems Wiki "Supported Matching Algorithms" confirms the letter map A=Allocation, F=FIFO, K=Configurable (two WebSearch summaries gave a wrong map; CME's table overrides them). Two refinements, neither changing the verdict: (a) TN and UB are no longer unconfirmed — CME's sheet shows both as F, so bucket (1) is stronger than the claim knew, and 3-Year Z3N outright is also F despite its spreads being K; (b) "a small resting order can be starved regardless of arrival time" mildly overstates — true of the pro-rata step (a share rounding below 2 lots yields zero there), but the top-order step rewards the first order to better the market and the residual step is FIFO, so arrival time is not wholly irrelevant on SR3. Freshness bound: P1 is dated Feb 2026 and no CME matching-algorithm change notice for these products was found through Aug 2026 (live cmegroup.com is WAF-blocked to curl and times out via WebFetch, so Feb 2026 is the latest obtainable snapshot). Full evidence table with all figures: C:\Users\chris\AppData\Local\Temp\claude\C--Users-chris-clee-ARBS\546c5bd0-3d16-4294-a405-eae00cac5d12\scratchpad\cme_match_algo_EVIDENCE.txt (supporting artifacts: globex_ref_2026.xls, refsheet_scan.py, SER-8867.pdf, understanding-sofr-futures.pdf in the same directory).
