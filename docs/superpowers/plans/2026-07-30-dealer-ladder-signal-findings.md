...
### The research question is answered and should be closed

Verdict (iii). The ladder does not predict SR3 or ZQ at this horizon and size, the mechanism is
unsupported, and the ordering runs the wrong way. **This is a successful outcome of the study as
specified** (§9c of the spec) and it does not warrant another pass at the same hypothesis on the
same data. Anything further would be re-specifying until something passes, which is what the
pre-registration exists to prevent.

The ladder remains useful as a **positioning observable** — what it was built for. Nothing here
argues against reading it; it argues against trading it at a one-hour horizon.

### Three data defects need owners, and none of them is this study's

Ordered by how much else they affect. **The first is a live production bug that would matter even
if this study had never happened.**

1. **PTP package grouping, dead since 2026-07-21** (§8.9). `ptp_group_id` NULL on every tape leg
   while `package_transaction_price` stays populated on 1,550–2,006 legs a day. Large multi-leg
   structures are being torn into standalone prints across the whole tape, not just here. The
   decisive test is ten minutes: run `group_by_ptp` on a raw 07-21 frame and print which of
   `pkg_ind_bool`, `ptp_usable | pts_usable` and `ts_all.notna()` is empty. Suspect the
   exec-vs-event timestamp migration (`169cb1a9`, 07-17) plus the
   `ptp16-exec-vs-event-timestamps` cache bump. See also the column normaliser at
   `usd_swaps.py:1971`, which lowercases before applying a CamelCase-split regex, so the regex
   can never match.
2. **2026-07-24 has no US cash session in the tape.** Needs an SDR re-ingest; not recoverable by
   re-running classification.
3. **The classification mid is displaced on 12 sessions** (§8.7). Inside our own curve stack, so
   it is a curve-build investigation rather than a ladder one.

### Two owner tickets left deliberately unactioned

- **`VINTAGE_SOURCES` omits 13 value-determining modules** (§8.8). Widening it changes the hash
  and would re-stamp a concluded six-month dataset as stale. Do it at the next backfill.
- **G0's label study reads six months in one statement.** Now chunked by month in
  `gates._load_units_chunked`; the equivalent read inside
  `SDRUtils/_swappulse_scripts/backfill_stir_ladder.py` is untouched because that file *is* in
  `VINTAGE_SOURCES`.

### The lockout is unspent, and is worth protecting

2026-06-10 → 07-29 has never been evaluated; `LOCKOUT_USED.json` does not exist. It is the one
genuinely clean out-of-sample window available, and it is **more** valuable now than before: a
future study on repaired data can use it exactly once, for real. Do not spend it re-testing a
hypothesis that already failed in-sample.

Note that its 34 sessions include the 13 that fail the data-quality gate, so repairing the
defects above materially improves what it can certify.

### What is reusable regardless of the verdict

- **`BT/dealer_ladder/session_quality.py`** — a per-session data-quality gate whose thresholds
  are measured rather than chosen. Off by default; 19 tests, mutation-checked.
- **`scripts/verify_findings_numbers.py`** — ties every figure quoted in prose to the artifact
  that produced it, and checks direction as well as digits.
- **`scripts/check_markdown_tables.py`** — splits table rows the way GFM does; found 109 broken
  rows a visual skim would not have.
- **The gate protocol itself** (G0–G5), and in particular the habit of writing what each outcome
  *would mean* before the numbers exist. It is why a null result here is publishable rather than
  ambiguous.

---

## 11. Every artifact, verbatim
