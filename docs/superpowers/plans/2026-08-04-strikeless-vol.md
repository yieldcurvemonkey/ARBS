...
**Amended a third time, after Tasks 20–22 — what the findings must say, and the numbers they must not misquote.**

*The denominator reversal, which must be shown and not buried.* Task 21 divided the breakeven by the **spread's** vol; the breakeven is a **parallel** move, so the comparator is the long leg's **rate** vol. Verified by repricing, not argument: on a real 2026-08-03 USD curve `package_dv01` is exactly 0.0, a 1bp parallel move gives dPV +101.88 against `½Γh² = +102.01` with an implied first-order term of 0.00, while a 1bp spread move is ~$100,000 of **first-order** P&L against ~$102 of convexity — 980×. Corrected, the valuation state **inverts in every market** (share-rich 97.9→10.8%, 72.2→16.9%, 74.6→12.6%, 74.4→1.2%) and the rule becomes a convexity **buyer**. `be_over_realized` now refuses a non-rate denominator; **do not pass `denominator=` to defeat it.** Show both the old and corrected numbers — a study that reversed its own conclusion under a single-variable correction reports both, or it is hiding the more interesting half.

*What that reversal changed, and what it did not.* Unchanged: all four markets DEAD (DSR 8.3e−30 / 3.1e−23 / 1.8e−16 / 2.5e−33 at 3888 trials), all losing at 1×, the two ranking signs disagreeing, the book failing to beat the best single market, and diversification arithmetic tightening to 0.2%. Reversed: **H8's central claim is CONFIRMED** — USD is the *nearest* to fair (BE/RV 0.684), not the furthest; JPY entering flatteners is **confirmed**; "three of four negative break-even multipliers" becomes two of four. **H8's EUR claim was never tested** — the drift veto was inert under the wrong denominator.

*The carry sign is zero evidence and must be stated as such.* A uniform carry sign across markets is an artefact of the rule's direction, not of the mechanism: the counterfactual "flattener on every held day" prints carry sign **−1 in all four markets**. It restates "BE/RV above threshold everywhere" and nothing more.

*H9 reporting.* Both constructions are rejected on their own claims. Report the **paired** median (+4.9%) led by the 62.1% win rate — the matched-pairs estimator — with median-of-levels (−1.6%) beside it labelled the weaker, unpaired one. After costs `two_leg` is best at every horizon tested, same-sector winning **0.0%** of dates at 21 days. What survives for same-sector is a **risk** statement, not a carry one: 28.5% less level+slope norm at identical cost. Pass `package_dv01_usd` to `carry_per_vega_after_costs` **explicitly** — its default is a magic 100_000.0 that no test exercises.

*Three honesty notes that must appear.* (1) `dd_per_vol_day` and Sharpe are **the same statistic on this data** (ratio 105.9–115.3 on every row), so "ranked on drawdown, not Sharpe" is vacuous here — say so rather than claiming an independent axis. (2) `n_trials = 3888` is a **lower bound**, not the full cross-family count; either state the full count or call it a lower bound. (3) A fly-hedged book is **not backtestable on the current engine** — `replication.simulate` charges per package while `cost_bp_round_trip` charges per traded leg DV01 — so the fly's rejection rests on theta, cost and neutralisation, which are independent of that convention. Record it; do not rebuild the engine to chase it.

*The methodological result, which belongs in the findings and not only in a ledger.* The DEAD verdicts survived the denominator error untouched — but **two of H8's three claims were reported refuted when they were in fact confirmed**, and what caught that was **not a test**. It was another task computing the same statistic on the same curves and getting a different answer. No unit test, no mutation, no review of Task 21 in isolation would have found it: the code was correct, the arithmetic was correct, and the label on the input was wrong in a way that only became visible when two independent implementations disagreed. Record what that implies for a research codebase — that agreement between independent recomputations catches a class of error that no amount of within-task rigour reaches, and that the study's other cross-checks (the analytic gamma control, the annuity identity, the two-code-path DV01 agreement) were doing the same job in the places where they existed.

*The bottom line.* The success criterion set at the outset was a modest standalone Sharpe with the right distribution and a working two-sided switch — explicitly *not* a high Sharpe from an untested static long. State plainly whether that was met. If nothing is ALIVE, say so in the first paragraph, not the last.

**Amended again — signal-input certification is REPORTED, not gated, and the notebook is what makes it visible.** `causal_signals` certifies `residual_z` end to end, and will certify `drift_t` and `iv_z` when handed their sources (a *builder* for the drift source, so the source's own causality is probed rather than assumed). `be_over_realized` — which sets the SIGN — and `spread_vol_bp_day` — which sets the SIZE — are still unaudited: the mechanism to check them is exported and nothing calls it.

My ruling on whether certification should gate ALIVE: **no, and the reason matters.** The six binding requirements are the spec's gate and this study does not get to invent a seventh mid-flight; and since two inputs currently have no caller, gating on full certification would make every row INELIGIBLE — honest, but it would destroy the information rather than report it. So:

* Print `certified_signal_inputs` and `uncertified_signal_inputs` **beside every league row**, from the signal frame's own `attrs`. A row whose `residual_z` is certified while its SIGN column is not is a row whose requirement 1 covers one of five inputs, and the table must say so rather than let `req_expanding_betas: True` imply more.
* **Call the exported mechanism for `be_over_realized` and `spread_vol_bp_day`** if their sources are on hand — it is cheap and it converts two reported gaps into two closed ones.
* Prefer the builder form for `drift_t`. `factors.changes_regression` is full-sample, and building the drift from it is the leaky construction this plan itself shipped until Task 18's review caught it; `expanding_changes_residual` is the causal one.
* The findings document must state, in plain words, how many of the five inputs were certified for the rows it quotes. "Requirement 1 met" is not the same claim as "the signal is causal", and four passes of this module confused exactly that kind of adjacent object for the real one.
