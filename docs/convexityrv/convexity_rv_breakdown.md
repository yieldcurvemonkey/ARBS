...
- **Convention traps (all measured in-house):** `CARRY_AND_ROLL_BPS_RUNNING` correlated **−0.136** with Citi's published 1y carry against a repriced `Curve.roll`'s **+0.991** — fixed 2026-08-27 (it aged a forward leg's tail instead of its start; `Query/IRSwaps/_carry_roll.py`) and now scores **+0.991 / 0.338 bp**. Horizon carry needs the accrual factor τ (the Nordea note omits it — literal transcription is 2× off). Forward-starting structures have **zero carry** at inception — their "carry" is pure static-curve roll.
