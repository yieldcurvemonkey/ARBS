...
class TestDBSCANClustering(unittest.TestCase):
    ...
    def test_generate_pairwise_combinations(self):
        ...
        cluster_sizes = [
            int(np.sum(self.pair_selector.clust_labels_ == label))
            for label in c_labels
        ]
        expected_pair_count = sum(size * (size - 1) // 2 for size in cluster_sizes)
        self.assertEqual(len(pair_list), expected_pair_count)
        self.assertEqual(len(pair_list), len(set(pair_list)))
        self.assertGreater(len(pair_list), 100)
