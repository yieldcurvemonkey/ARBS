...
class Gerber:
    ...
    def _setup(self, rtns: pd.DataFrame, threshold: float = 1 / 2):
        ...
        self.hk = self.stds.rename(columns={self.stds_cols[0]: "ticker", self.stds_cols[1]: "h_k"}).assign(h_k=lambda x: x.h_k * threshold)

        self.threshold_df = (
            self.rtns.reset_index().melt(id_vars=self.rtns.index.name, var_name="ticker", value_name="rtns").merge(right=self.hk, how="inner", on="ticker")
        )

    def rolling_gerber_corr(self, rtns: pd.DataFrame, window: int, threshold: float = 1 / 2) -> pd.DataFrame:

        df_compare = (
            rtns.rolling(window=window)
            .std()
            .dropna()
            .reset_index()
            ...
            .drop(columns=["_std"])
            .merge(right=rtns.reset_index().melt(id_vars="date", value_name="rtns"), how="inner", on=["date", "variable"])
        )

        combined_tmp = (
            df_compare.melt(id_vars=["date", "variable"], var_name="stat", value_name="num")
    ...
    def _get_stat(self, combined_tmp: pd.DataFrame, combo) -> pd.DataFrame:
        ...
        combined_tmp = combined_tmp.fillna(0)[["m_ij"]].assign(ticker1=combo[0], ticker2=combo[1])
    ...
    def _get_combined_tmp(self):
        ...
        for combo in combinations:
            ...
            returns_tmp = (
                self.threshold_df[[self.rtns.index.name, "ticker", "rtns"]]
                .query("ticker == [@combo[0], @combo[1]]")
                .pivot(index=self.rtns.index.name, columns="ticker", values="rtns")
                .rename(columns={combo[0]: "{}_rtns".format(combo[0]), combo[1]: "{}_rtns".format(combo[1])})
            )

            hk_rtmp = (
                self.threshold_df[[self.rtns.index.name, "ticker", "h_k"]]
                .query("ticker == [@combo[0], @combo[1]]")
                .pivot(index=self.rtns.index.name, columns="ticker", values="h_k")
                .rename(columns={combo[0]: "{}_hk".format(combo[0]), combo[1]: "{}_hk".format(combo[1])})
            )
    ...
    def corr(self, rtns: pd.DataFrame, threshold: float = 1 / 2, method="method1"):
        ...
        if method == "method2":
            ...
            gerber_tmp = pd.DataFrame({"ticker1": [combo[0]], "ticker2": [combo[1]], "gerber_stat": [sum(combined_tmp["m_ij"]) / sum(abs(combined_tmp["m_ij"]))]})
    ...
    def cov(self, rtns: pd.DataFrame, threshold: float = 1 / 2, method: str = "method1"):
        ...
        self.cov_mat = pd.DataFrame(
            data=np.dot(np.diag(self.std_vec), np.dot(self.corr_mat, np.diag(self.std_vec))), columns=self.corr_mat.columns, index=self.corr_mat.columns
        )
    ...
    def _get_lag(self, rtns: pd.Series, lags: int = 30) -> pd.DataFrame:
        ...
        df_prep = rtns.to_frame().sort_values(rtns.index.name).rename(columns={rtns.name: "value"})

        for lag in range(lags):
            df_prep[lag + 1] = df_prep["value"].shift(lag + 1)
    ...
    def rolling_gerber_OLS(
        self, endog: pd.Series, exog: pd.Series, window: float, verbose: bool = False, threshold: float = 1 / 2, method: str = "method1"
    ) -> pd.DataFrame:

        rtns = endog.to_frame().merge(right=exog, how="inner", on=endog.index.name).rename(columns={endog.name: "endog", exog.name: "exog"})

        df_out = pd.DataFrame({"alpha": [], "beta": []})
    ...
    def cumsum_comovement(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return cumsum.to_frame().rename(columns={0: "cumsum"})
