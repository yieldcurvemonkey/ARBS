...
#    p 0.008**, against the JPM corpus's **+13w, r 0.737, p 0.008**. Two
...
#    **p 0.4120**, and a paired plateau covering **all 27 lags**. Going from 147
...
# 7. **The sub-periods disagree about where, and only the recent one is
#    significant.** Five ~4-year blocks: levels argmax +9, +11, −13, +13, +13;
#    changes +9, +2, −13, +2, +13; p from **0.004 to 0.833**. Two of the ten
#    cells clear 0.05 — **2017-2021 levels (p 0.006**, on the transform whose
#    null fires 12-20% of the time**)** and **2021-2027 changes (+13w, r 0.160,
...
#
# **One correction to the shared estimator landed while this study was being
# built, and it moved both studies' p-values.** The rotation null re-scans every
# lag after rotating, so a rotation smaller than *twice* the widest lag still had
# the true alignment inside its scan window and reproduced the observed statistic
# instead of testing against it. On this hole-free 21-year sample every single
# "exceedance" was one of those self-matches, which made the p a function of
# where the argmax sat rather than of how large the correlation was. Fixed by
# requiring `min_offset > 2 * max_lag`; the numbers below are post-fix.
#
...
# study 1's figures below are its CORRECTED ones, after the rotation null's
# self-matching bug was fixed in the shared module (the fix raises every p that
# the self-matches had inflated).
for tr, s1 in (("levels", "PIT +13w r 0.737 p 0.008 | as-pub +12w r 0.652 p 0.008"),
               ("changes", "PIT +13w r 0.153 p 0.475 | as-pub  +0w r 0.104 p 1.000")):
...
_sig = [(r["block"], t, r[f"{t}_p"]) for _, r in STAB.iterrows()
        for t in ("levels", "changes") if r[f"{t}_p"] < 0.05]
print(f"\n{len(_sig)} of {2*len(STAB)} block-transform cells clear 0.05: {_sig}")
print(f"(p ranges {min(STAB['levels_p'].min(), STAB['changes_p'].min()):.3f} to "
      f"{max(STAB['levels_p'].max(), STAB['changes_p'].max()):.3f}.) The levels argmax")
print("visits +9, +11, -13, +13, +13, so 'where' is not stable. The one changes cell")
print("that clears is 2021-2027 at +13w -- the block holding study 1's window, and it")
print("agrees with study 1's horizon. The pooled +2w result is not visible in any")
print("single block, which is what a small persistent effect looks like -- and also")
print("what a pooled artefact looks like, which is why it is reported at r 0.12 and")
print("not called a signal.")
