...
Usage::
    ...
    python notebooks/rv/_audit_fed_expected_sentiment_numbers.py [notebook.ipynb]
"""
...
DEFAULT_NB = HERE / "fed_expected_sentiment_rv.ipynb"
...
WHITELIST = {
    ...
    "0.12": "fedlock_sentiment_lead: the 21-year correlation at the surviving lead",
    "0.7": "fed_sentiment_lead / fedlock_sentiment_lead: the 2023-2026 correlation "
           "at the measured lead, cited as a lower bound rather than re-measured",
    "0.219": "project_global_cb_hawk_dove: the intraday FED leg, bp per trade",
    "2048": "fed_detachment_rv: its cell count, cited for contrast",
    "47.2": "project_citivelo_tagcache_vol_poison: the share of the 2y tag that is vol",
    "47": "the same share, quoted to the nearest per cent",
    # ---- process constants and structural integers ----
    # Everything a cell computes is deliberately NOT here: the tie-out cell at
    # the foot of the notebook prints each of those, so the audit checks them
    # rather than excusing them.
    **{t: "structural" for t in (
        # the lead axis and the horizon axis
        "11", "13", "0.5", "1.0",
        # calendar years the prose names
        "2026", "2025", "2024", "2023", "2018", "2005",
        # the cost convention
        "0.25", "0.50",
