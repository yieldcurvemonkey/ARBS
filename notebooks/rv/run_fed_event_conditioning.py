"""Build, execute, verify and audit the Fedspeak event-conditioning notebook.
...
Usage::
    ...
    conda run -n stir python notebooks/rv/run_fed_event_conditioning.py --no-exec

**Run the grids first.** The notebook reads
``fed_event_conditioning_results.pkl`` and does no searching of its own, so it
...
STEM = "fed_event_conditioning_rv"
...
def main(argv=None) -> int:
    ...
    audit = _run([sys.executable, str(HERE / "_audit_fed_event_conditioning_numbers.py"),
                  str(nb)], REPO)
