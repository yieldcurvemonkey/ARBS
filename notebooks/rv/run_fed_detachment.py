"""Build, execute, verify and audit the Fedspeak-detachment notebook.
...
Usage::
    ...
    conda run -n stir python notebooks/rv/run_fed_detachment.py
    conda run -n stir python notebooks/rv/run_fed_detachment.py --no-exec

Budget **forty to fifty minutes**. The run is dominated by three grid searches
with their rotation and phase nulls, plus the harness's own size and power
calibration -- somewhere north of a hundred thousand grid evaluations.
``timeout=None`` is deliberate: nbclient's 30-second default kills every one of
those cells.
"""
...
def main(argv=None) -> int:
    ...
    audit = _run([sys.executable, str(HERE / "_audit_fed_detachment_numbers.py"),
                  str(nb)], REPO)
