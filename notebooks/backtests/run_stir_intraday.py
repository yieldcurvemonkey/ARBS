"""Build, execute and verify the SR3 4-hour intraday lab.
...
Usage::
    ...
    conda run -n stir python notebooks/backtests/run_stir_intraday.py
    conda run -n stir python notebooks/backtests/run_stir_intraday.py --as-scripts
"""
...
NOTEBOOKS = ["stir_intraday_kink"]
DATA = REPO / "notebooks" / "data" / "stir_intraday"

#: every CSV the lab writes, cleared before each run so a stale table from a
#: renamed section cannot survive and be read as current.
OUTPUTS = ("reconciliation.csv", "pond_all_slots.csv", "pond_front_legs.csv",
           "min_holding_period.csv", "variance_ratios.csv",
           "execution_timing.csv", "execution_paired.csv",
           "reversion_demeaned.csv", "fomc_bar.csv")
