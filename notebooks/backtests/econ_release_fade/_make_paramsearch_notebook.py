...
placebo cell, the search found the shape of a search. Comparing the two *distributions* is stronger
still, because it does not depend on which single cell happened to win.
""")
...
k = ks_2samp(elig.sr_per_trade.dropna(), eligp.sr_per_trade.dropna())
print(f"real against placebo, bp/trade : Mann-Whitney p = {u.pvalue:.3e}")
print(f"real against placebo, Sharpe   : KS p = {k.pvalue:.3e}")
...
# Paired, config by config: the strongest form of the comparison.
pair = elig[["avg_bp"]].join(eligp[["avg_bp"]], how="inner", lsuffix="_real", rsuffix="_plac")
print(f"\n{len(pair):,} configurations present in both grids; "
      f"real beats its own placebo twin in {float((pair.avg_bp_real > pair.avg_bp_plac).mean()):.1%}")

fig, axes = plt.subplots(1, 3, figsize=(20, 4.4))
axes[0].hist(eligp.sr_per_trade, bins=60, alpha=.6, density=True, color="grey", label="wrong day")
axes[0].hist(elig.sr_per_trade, bins=60, alpha=.6, density=True, color="seagreen", label="real")
axes[0].axvline(0, color="k", lw=.8); axes[0].legend(fontsize=8)
axes[0].set_xlabel("Sharpe per trade"); axes[0].set_title("the two searches, compared as distributions")
axes[1].hist(eligp.avg_bp, bins=60, alpha=.6, density=True, color="grey", label="wrong day")
axes[1].hist(elig.avg_bp, bins=60, alpha=.6, density=True, color="seagreen", label="real")
...
axes[1].set_title("edge per trade")
if len(pair):
