...
class CMEFetcher(BaseFetcher, ZODBCacheMixin):
    ...
    def build_rl_eod_curves(
        ...
        assert type == "Df", "only discount factors supported in rateslib-cme curve build"
        ...
        for curve_date, curve_report_df in curve_iter:
            ...
            try:
                ...
                rl_curves_dict[curve_date] = rl_discount_curve
            except Exception as e:
                self._logger.error(f"Error when building RatesLib Curve for CME {curve} on {curve_date}: {e}")
