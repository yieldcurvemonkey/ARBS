...
def detect_swaption_straddles_df(
    ...
    *,
    ...
    ignoring the must_be_reported_as_package flag.
    ...
    Args:
        ...
        custy_leg_option_premium_amount_tolerance: Percentage tolerance for matching
            customer (BILT) trades based on option_premium_amount (e.g., 0.05 = 5%).
            When provided, enables additional matching rule for customer trades.
        option_premium_amount_col: Column name for option premium amount (default: "option_premium_amount")
    ...
    Returns:
        ...
        For customer straddle matches, the option_premium_amount column will contain
        the sum of both legs' premium amounts.
    """
    if df.empty:
        return df
    ...
    if custy_leg_option_premium_amount_tolerance is not None:
        ...
        is_bilt = out[config.platform_col] == "BILT"
        ...
        if not custy_payers.empty and not custy_receivers.empty:
            # Parse option_premium_amount for matching
            if option_premium_amount_col in custy_payers.columns:
                custy_payers["_opt_prem"] = custy_payers[option_premium_amount_col].apply(_safe_float)
            else:
                custy_payers["_opt_prem"] = np.nan

            if option_premium_amount_col in custy_receivers.columns:
                custy_receivers["_opt_prem"] = custy_receivers[option_premium_amount_col].apply(_safe_float)
            else:
                custy_receivers["_opt_prem"] = np.nan

            custy_payers["_t"] = _ensure_int64_epoch_seconds(custy_payers[config.exec_col])
            custy_receivers["_t"] = _ensure_int64_epoch_seconds(custy_receivers[config.exec_col])
            ...
            for cp_idx in custy_payer_idx:
                ...
                cp_t = cp_row["_t"]
                cp_strike = _safe_float(cp_row.get(config.strike_col))
                cp_expiry = cp_row.get(config.expiration_col)
                cp_tenor = _safe_float(cp_row.get(config.tenor_col))
                cp_notional = _safe_float(cp_row.get(config.notional_col))
                cp_platform = cp_row.get(config.platform_col, "")
                cp_currency = cp_row.get(config.currency_col, "")
                cp_underlier = cp_row.get(config.underlier_col, "")
                cp_trade_id = str(cp_row.get(config.trade_id_col, ""))
                ...
                for cr_idx in custy_receiver_idx:
                    ...
                    cr_strike = _safe_float(cr_row.get(config.strike_col))
                    cr_expiry = cr_row.get(config.expiration_col)
                    cr_tenor = _safe_float(cr_row.get(config.tenor_col))
                    cr_notional = _safe_float(cr_row.get(config.notional_col))
                    cr_platform = cr_row.get(config.platform_col, "")
                    cr_currency = cr_row.get(config.currency_col, "")
                    cr_underlier = cr_row.get(config.underlier_col, "")
                    ...
                    # Check economic filters
                    if config.require_same_platform and cp_platform != cr_platform:
                        continue
                    if config.require_same_currency and cp_currency != cr_currency:
                        continue
                    if config.require_same_underlier and cp_underlier != cr_underlier:
                        continue
                ...
                if best_custy_match is not None:
                    ...
                    cr_row = custy_receivers.loc[cr_idx]
                    cr_trade_id = str(cr_row.get(config.trade_id_col, ""))
                    cr_opt_prem = cr_row["_opt_prem"]
                    ...
                    if abs(_safe_float(cp_row.get(config.strike_col)) - _safe_float(cr_row.get(config.strike_col))) < 0.00001:
                        custy_confidence += 0.1
                    ...
                    custy_reason = _build_package_reason(
    ...
    *,
    ...
    Args:
        ...
        pricer: Optional QLIRSwapCurve instance for QuantLib-based vega calculation.
            If provided, uses rigorous pricing instead of premium-based heuristic.
        curve_provider: Optional callable that takes an execution timestamp and returns
            a QLIRSwapCurve for that date. Useful for historical analysis with varying
            market data. Signature: (datetime) -> Optional[QLIRSwapCurve]
        use_quantlib_vega: If True and pricer/curve_provider is available, use QuantLib
            pricing for vega calculation. If False, always use premium-based heuristic.
    ...
    Returns:
        ...
        - vega_curve_pricing_method: "QUANTLIB" or "PREMIUM_HEURISTIC"
    """
    if df.empty:
        return df
    ...
    agg_dict: Dict[str, Any] = {
        ...
        config.premium_col: "sum",  # Total premium (vega proxy fallback)
    ...
    for i in range(n_straddles):
        ...
        for j in range(i + 1, n_straddles):
            ...
            vega_j, method_j = straddle_vegas.get(pid_j, (0.0, "PREMIUM_HEURISTIC"))
