"""
USD SOFR swaps ingestion for SwapPulse.

Persists USD SOFR swap classifications to normalized package/leg tables and
...
PACKAGES_TABLE = "arbs_sofr_swap_packages_v1"
LEGS_TABLE = "arbs_sofr_swap_legs_v1"
RUNS_TABLE = "arbs_sofr_swap_ingestion_runs_v1"
MANUAL_LINKS_TABLE = "arbs_sofr_swap_manual_links_v1"
LINK_HISTORY_TABLE = "arbs_sofr_swap_link_history_v1"
DISPLAY_VIEW_V1 = "arbs_sofr_swap_display_items_v1"
DISPLAY_VIEW_V2 = "arbs_sofr_swap_display_items_v2"
...
CREATE INDEX IF NOT EXISTS idx_sofr_swap_packages_type_date ON {PACKAGES_TABLE}(package_type, as_of_date);
CREATE INDEX IF NOT EXISTS idx_sofr_swap_packages_exec ON {PACKAGES_TABLE}(execution_start);
CREATE INDEX IF NOT EXISTS idx_sofr_swap_packages_source ON {PACKAGES_TABLE}(package_source);
CREATE INDEX IF NOT EXISTS idx_sofr_swap_legs_package ON {LEGS_TABLE}(package_id);
CREATE INDEX IF NOT EXISTS idx_sofr_swap_legs_exec ON {LEGS_TABLE}(execution_timestamp);
CREATE INDEX IF NOT EXISTS idx_sofr_swap_legs_manual_link ON {LEGS_TABLE}(manual_link_id);
CREATE INDEX IF NOT EXISTS idx_sofr_swap_manual_links_trades ON {MANUAL_LINKS_TABLE} USING GIN (linked_trade_ids);
CREATE INDEX IF NOT EXISTS idx_sofr_swap_manual_links_active ON {MANUAL_LINKS_TABLE}(is_active) WHERE is_active = TRUE;
CREATE INDEX IF NOT EXISTS idx_sofr_swap_manual_links_created ON {MANUAL_LINKS_TABLE}(created_at);
CREATE INDEX IF NOT EXISTS idx_sofr_swap_manual_links_manual_pkg ON {MANUAL_LINKS_TABLE}(manual_package_id);
CREATE INDEX IF NOT EXISTS idx_sofr_swap_link_history_link ON {LINK_HISTORY_TABLE}(link_id);
...
def build_classification_dataframe(
    ...
    """
    Build the SOFR swap classification DataFrame at leg grain.
...
def main(
    ...
    if raw_df.empty:
        print("No SOFR swap trades found in date range.")
        return
...
def ingest_incremental_once(
    ...
    *,
    ...
    print("Incremental SOFR swap ingestion")
...
def main_service(
    ...
    print("Starting SOFR swap ingestion service")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest USD SOFR swap classifications into SwapPulse Postgres.")
    parser.add_argument(
        "--mode",
