...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error('sofr-swap manual links GET error', error)
    return NextResponse.json(
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const linkedTradeIds = Array.from(new Set(legs.map((leg) => leg.trade_id)))
    const conflicts = await findManualLinkConflicts(linkedTradeIds)
    const { items, hasErrors } = buildValidationItems(legs, metrics, conflicts)
    ...
    await applyManualLinkToTrades(linkResult.link_id, linkedTradeIds)
    ...
    console.error('sofr-swap manual links POST error', error)
    return NextResponse.json(
