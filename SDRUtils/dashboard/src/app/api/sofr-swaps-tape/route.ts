// ABOUTME: Reads SOFR swap tape display rows with cursor pagination and column filters.
...
const FLAGS_FILTER_EXPRESSIONS = [
  "coalesce(d.package_metrics->>'is_mac_any', 'false')",
  "coalesce(d.package_metrics->>'is_spreadover_any', 'false')",
  "coalesce(d.package_metrics->>'is_asset_swap_any', 'false')",
  "coalesce(d.package_metrics->>'matched_ust_any', 'false')",
  'd.legs_json::text'
]
...
const COLUMN_FILTER_EXPRESSIONS_V2: Record<string, string[]> = {
  ...
  flags: FLAGS_FILTER_EXPRESSIONS
}
...
function buildNumericCondition(
  ...
  if (Number.isNaN(numericValue)) {
    return buildTextCondition([`${expression}::text`], matchMode, rawValue, params)
  }
...
function buildColumnFilterClause(
  ...
  return buildTextCondition(expressions, constraint.matchMode, constraint.value, params)
}
...
export async function GET(request: Request) {
  ...
  const columnFilters = parseColumnFilters(searchParams.get(COLUMN_FILTER_QUERY_KEY))
  ...
  if (filter) {
    ...
    const filterClauses = [
      ...
      `d.package_type ILIKE $${idx}`,
      `d.legs_json::text ILIKE $${idx}`
    ]
    if (hasManualFields) {
      filterClauses.push(`d.manual_package_id ILIKE $${idx}`)
      filterClauses.push(`d.user_comment ILIKE $${idx}`)
      filterClauses.push(`d.link_reason ILIKE $${idx}`)
    }
  ...
  try {
    ...
    let rows = result.rows as SofrSwapTapeRow[]
    let hasMore = false
    ...
    const nextCursor =
      hasMore && rows.length > 0 ? rows[rows.length - 1].execution_start : null

    const latestExecutionStart = rows.length > 0 ? rows[0].execution_start : since || null
    ...
    console.error('sofr-swaps-tape GET error', error)
    return NextResponse.json(
      { error: error?.message || 'Failed to fetch SOFR swaps tape' },
      { status: 500 }
    )
  }
}
