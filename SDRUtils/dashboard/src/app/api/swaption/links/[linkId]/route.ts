// ABOUTME: Manage a single manual link (detail, update, deactivate).
// ABOUTME: Provides history and linked trade details for manual links.
...
export async function GET(
  ...
  try {
    ...
    console.error('manual link GET error', error)
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    const removeLegs = removeIdsRaw.length
      ? await resolveLinkLegs(removeIdsRaw)
      : []
    ...
    const nextTradeIds = Array.from(
      new Set(nextLegs.map((leg) => leg.trade_id))
    )
    ...
    if (payload.package_type !== undefined)
      changeDetails.package_type = nextPackageType
    if (payload.comment !== undefined) changeDetails.user_comment = nextComment
    if (payload.link_reason !== undefined)
      changeDetails.link_reason = nextReason
    if (payload.tags !== undefined) changeDetails.tags = nextTags
    if (addTradeIds.size) changeDetails.add_trades = Array.from(addTradeIds)
    if (removeTradeIds.size)
      changeDetails.remove_trades = Array.from(removeTradeIds)
    ...
    console.error('manual link PATCH error', error)
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    console.error('manual link DELETE error', error)
    return NextResponse.json(
