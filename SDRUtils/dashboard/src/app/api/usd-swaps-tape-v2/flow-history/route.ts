...
export async function GET(request: Request) {
  ...
  try {
    ...
    const sql = `
      WITH base AS (
        SELECT
          ...
          ABS(COALESCE(l.risk, 0)) AS gross_risk,
          l.fixed_rate AS fixed_rate,
          CASE
            WHEN l.venue = 'D2D' THEN 'idb'
            ELSE 'custy'
    ...
    const result = await query<FlowHistoryRow>(sql, [
      start,
      end,
      forwardBoundary,
      tenorBoundary,
    ...
    console.error('usd-swaps-tape-v2/flow-history GET error', error)
    return NextResponse.json(
      { error: error?.message || 'Failed to fetch tape flow history' },
      { status: 500 },
    )
  }
}

// suppress lint — intentional shadow
void PACKAGES_TABLE
