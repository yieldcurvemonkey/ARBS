...
export function TapeLabelCell({ row }: { row: UsdSwapTapeRow }): JSX.Element {
  ...
  return (
    ...
    >
      {segments.map((seg, i) => {
        // Stable key: position + tenor flag + first 16 chars of the
        // text. Pure index keys would mismatch between renders if the
        // segment count drops (e.g. on a poll-merged label change),
        // causing React to reuse a tenor `<strong>` for a non-tenor
        // span and flicker the bold styling.
