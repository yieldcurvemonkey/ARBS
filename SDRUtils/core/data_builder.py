"""
SDR Data Builder - Core data fetching and caching engine.

Provides async/concurrent data fetching from DTCC SDR and
intelligent caching for historical and intraday data.
"""
...
def datetime_today_utc():
    """Get current UTC date as datetime."""
    return datetime(
...
class DTCCFetcher(BaseFetcher):
    ...
    Provides async/concurrent fetching of SDR reports from DTCC pddata.
    Handles both historical (cumulative) and intraday (slice) reports.
    """
    ...
    async def _fetch_dtcc_sdr_data_helper(
        ...
        dtcc_sdr_url, dtcc_sdr_header = self._get_dtcc_url_and_header(
            date_string=date_string, agency=agency, asset_class=asset_class
        )
        ...
        while retries < max_retries:
            try:
                ...
                self._logger.debug(
                    f"DTCCFetcher - HTTP Error {e.response.status_code} for {agency}-{asset_class}-{date_string}. "
                ...
                wait_time = backoff_factor * (2 ** (retries - 1))
                self._logger.debug(
                    f"DTCCFetcher - Connection/Other Error {e} for {agency}-{asset_class}-{date_string}. "
                    f"Retrying in {wait_time}s..."
                )
                await asyncio.sleep(wait_time)
    ...
    def _extract_dataframes_from_zip(
        ...
        with pyzipper.AESZipFile(zip_buffer) as zip_file:
            ...
            candidates = [
                ...
                if not info.is_dir() and info.filename.lower().endswith(allowed_extensions)
            ]
            if not candidates:
                return {}
    ...
    def fetch_intraday_reports(
        ...
        slice_ids = self._get_dtcc_intraday_slide_ids(
            agency=agency, asset_class=asset_class, start_timestamp=start_timestamp, end_timestamp=end_timestamp
        )
...
def _save_daily_dict(
    ...
    *,
    ...
    """Save daily data dict to parquet files."""
    base = Path(base_dir)
...
def _load_daily_dict(
    ...
    *,
    ...
    """Load daily data dict from parquet files."""
    base = Path(base_dir)
...
def _concat_dfs(
    ...
    *,
    ...
    """
    Hyper-optimized DataFrame concatenation with deduplication.

    Optimizations:
    - One-pass iteration over frames
    - Applies filter per-frame
    - Streaming global de-dup for single column keys
    """
...
class SDRDataBuilder:
    """
    Main SDR Data Builder class.

    Provides high-level API for fetching SDR trade data with intelligent
    caching for both historical and intraday data.
    """
    ...
    def grab_intraday_sdr_trades(
        ...
        """Fetch intraday SDR trades with caching."""
        start_timestamp = pd.to_datetime(start_timestamp, utc=True)
        end_timestamp = pd.to_datetime(end_timestamp, utc=True)
        ...
        combined = (
            ...
            .drop_duplicates(subset=["report_slice", ts_col])
    ...
    def grab_historical_sdr_trades(
        ...
        """Fetch historical SDR trades with caching."""
        all_days = list(
            pd.date_range(start=start_date, end=end_date, freq=CustomBusinessDay(calendar=USFederalHolidayCalendar())).date
        )
        cached_days: Dict[date, pd.DataFrame] = {}
        to_fetch = all_days

        cached_days = _load_daily_dict(
            all_days, self._parquet_cache_dir, agency=agency, asset_class=asset_class, show_tqdm=self._show_tqdm
        )
    ...
    def grab_sdr_trades(
        ...
        *,
        ...
        """
        Fetch SDR trades for a timestamp range.

        Combines historical and intraday data as needed.
        """
