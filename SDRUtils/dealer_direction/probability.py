...
``b0 := median(trimmed sample)``** (:func:`_anchored_decomposition`) -- 27 of
27 buckets with n >= 300, and ``b0 == median(trimmed)`` to float equality in
every row of the table above (re-measured, ``scratch/pf07_real.py``). F-20's
...
def _em(x, b0: float, h: float, s: float, max_iter: int, tol: float):
    ...
    for _ in range(max_iter):
        ...
        if h_new < 0.0:
            # Label swap. The likelihood is invariant (pinned in
            # ``test_the_likelihood_is_invariant_under_the_label_swap_the_em_relies_on``)
            # and h > 0 is the convention, so this renames the components
            # rather than changing the answer -- which is why b0_new is
            # recomputed with the sign put back: under h -> -h the
            # responsibilities flip too, u -> -u, and the two negations cancel
            # in ``b0 = xbar - h ubar``.
            #
            # UNREACHABLE FROM THESE STARTS, and kept as a guard rather than as
            # live code: u = tanh(h (x - b0) / s^2) is increasing in x for
            # h > 0, so cov(u, x) >= 0 and h_new is never negative; every start
            # in `fit_mixture` has h > 0. A mutation raising inside this branch
            # leaves the suite green, and that is the correct report, not a
            # coverage hole to be papered over with a contrived call.
            h_new = -h_new
            b0_new = xbar - (-h_new) * ubar
        d = x - b0_new
...
__all__ = [
    ...
    "MixtureFit", "MomentCheck", "ProbabilityCall", "REPORT_COLUMNS",
    "SpreadCrossCheck",
