...
WHAT HAS BEEN CHECKED AGAINST A NUMBER THIS MODULE DID NOT PRODUCE
------------------------------------------------------------------

``b0`` reproduces LEDGER F-20's independently measured Barchart tenor gradient
on the legacy deviations (``scratch/prob05_real.py``), which is the closest
thing to a known answer real data offers::
...
Sign, monotone shape and magnitude all reproduce; the mild compression at 2Y-3Y
is the trim removing an asymmetric tail that the raw median keeps.
...
#: Minimum sample size when ``h`` comes from outside the sample (the tick
#: lattice sub-3y, or the parent bucket). Less than half the unanchored floor,
#: because only ``s`` is being estimated and it comes out of one second moment
#: rather than a three-parameter likelihood. Measured the same way --
...
#:
#: ``TAU_RECOVERY_TOLERANCE / 1.2816`` -- the p90 of a symmetric error is
#: 1.2816 standard errors, so this is the tolerance expressed as an SE.
...
def fit_mixture(
    ...
    *,
    ...
    ``tick_stats`` and ``parent_h`` are the fallbacks for a bucket that fails
    the moment check -- see :data:`FIT_ROBUST_FALLBACK`.
    """
    ...
    # An h from outside this sample -- the tick lattice sub-3y, or the parent
    # bucket -- changes what has to be estimated, and therefore how much data
    # it takes. See MIN_BUCKET_N's docstring for why that matters so much.
    anchor_h = None
    if tick_stats is not None:
        anchor_h = _independent_half_spread(tick_stats)
    if anchor_h is None and parent_h is not None and parent_h > 0:
        anchor_h = float(parent_h)
    min_n_given = min_n
    flags: list[str] = []
    ...
    if FIT_LEPTOKURTIC in flags or unseparated or imprecise:
        if anchor_h is not None:
            ...
            b0, h, s, extra = _anchored_decomposition(xt, anchor_h)
...
def _anchored_decomposition(xt, h_anchor: float, external: bool = True):
    ...
    is what a leptokurtic bucket with no tick and no parent gets. Emitting
    ``FIT_ANCHORED_H`` there would claim an independent estimate that does not
    exist, and -- worse than the cosmetic lie -- it would grant the bucket
    ...
    strength of the one number in it that cannot be trusted.
    """
...
def tenor_band(years) -> str:
    ...
    return TENOR_BAND_LABELS[0] if y <= 0 else TENOR_BAND_LABELS[-1]
...
def _parent_h(fits: dict, dims: dict, order: tuple[str, ...]) -> float | None:
    """The already-fitted parent's ``h``, as an anchor for a thin child.

    Only a parent whose own fit was not itself forced is offered: passing down
    an ``h`` that was pinned at the no-information floor would propagate the
    floor through the whole subtree and turn one unidentifiable bucket into a
    branch of them.
    """
    for dropped in order:
        if dropped not in dims:
            continue
        parent = _label({k: v for k, v in dims.items() if k != dropped})
        f = fits.get(parent or GLOBAL_BUCKET)
        if f is None:
            f = fits.get(GLOBAL_BUCKET)
        if f is not None and FIT_UNSEPARATED not in f.flags and f.h > 0:
            return float(f.h)
        return None
    return None
...
class Calibration:
    ...
    def fit(cls, df: pd.DataFrame, *, order: tuple[str, ...] = DEFAULT_POOLING_ORDER,
        ...
        # Coarsest first, so that a fine bucket with no tick estimate of its
        # own can borrow its parent's fitted h as an anchor. Going the other
        # way would need the parent before it exists.
        ...
        for cols in reversed(levels):
            ...
            for keyvals, g in df.groupby(list(cols), dropna=False):
                ...
                fits[label] = fit_mixture(
                    ...
                    trim_k=trim_k, min_n=min_n, tick_stats=ts,
                    parent_h=_parent_h(fits, dims, order), as_of=as_of)
    ...
    * **rolling** has neither problem, at the cost of refitting. Cost is the
    ...
    for i, as_of in enumerate(dates):
        ...
        if not sel.any():
            continue
        window = df.loc[sel]
        if len(window) < fit_kwargs.get("min_n", MIN_BUCKET_N):
            continue
...
def tau_stability(fits, *, alpha: float = 0.05, method: str = "chi2",
    ...
    if len(usable) < 2:
        raise ValueError("tau_stability needs at least two fits with standard errors")
    ...
    if method == "chi2":
        ...
        p = _bootstrap_q_pvalue(usable, q, reps=reps, seed=seed)
    else:
...
def _bootstrap_q_pvalue(fits, q_obs: float, *, reps: int, seed: int) -> float:
