...
from Query.USTFutureOptions import adapter as _ustfo_adapter  # noqa: F401
...
class USTFutureOptionQuery(BaseQuery):
    structure: USTFutureOptionStructure = USTFutureOptionStructure.OUTRIGHT
    value: Union[USTFutureOptionValue, List[USTFutureOptionValue]] = USTFutureOptionValue.PRICE
    ...
    product: str = field(init=False, default="USTFUTUREOPTION")
    structure_id: Any = field(init=False, default=None)

    def __post_init__(self):
        object.__setattr__(self, "product", "USTFUTUREOPTION")
        ...
        if self.structure == USTFutureOptionStructure.OUTRIGHT:
            assert skw.get("symbol") is not None, "OUTRIGHT requires symbol"
        elif self.structure == USTFutureOptionStructure.VERTICAL:
            ...
            assert (
                skw.get("long_symbol") is not None and skw.get("short_symbol") is not None
            ), "VERTICAL requires long_symbol and short_symbol"
            skw.setdefault("risk_weights", [1.0, -1.0])
        elif self.structure == USTFutureOptionStructure.STRADDLE:
            if skw.get("symbol") is not None:
                ...
                assert (
                    skw.get("call_symbol") is not None and skw.get("put_symbol") is not None
                ), "STRADDLE requires symbol or call_symbol+put_symbol"
            skw.setdefault("risk_weights", [1.0, 1.0])
    ...
    def return_query(self) -> List["USTFutureOptionQuery"]:
        if isinstance(self.value, list):
    ...
    def col_name(self, cube_name: Optional[str] = None) -> str:
        ...
        if self.structure == USTFutureOptionStructure.VERTICAL:
    ...
    def default_mtm_value_id(self):
        return USTFutureOptionValue.PRICE
