...
class TestPerBlockShrinkage:
    """Test per-block Ledoit-Wolf shrinkage for sector-based models."""

    def test_per_block_shrinkage_reduces_condition_number(self):
        """Per-block shrinkage should reduce condition number of each block."""
        from Risk.Covariance.LedoitWolfShrinkage import LedoitWolfShrinkage

        np.random.seed(42)

        # Create two blocks with different characteristics
        # Block 1: Technology (higher variance, lower correlation)
        tech_returns = pl.DataFrame(
            np.random.randn(50, 5) * 2.0,
            schema=['AAPL', 'MSFT', 'GOOGL', 'META', 'NVDA']
        )

        # Block 2: Utilities (lower variance, higher correlation)
        util_common = np.random.randn(50, 1)
        util_idio = np.random.randn(50, 3) * 0.5
        util_returns = pl.DataFrame(
            0.8 * util_common + 0.2 * util_idio,
            schema=['DUK', 'SO', 'NEE']
        )

        # Create blocks dict (use numpy.cov for proper covariance computation)
        blocks = {
            'Technology': np.cov(tech_returns.to_numpy(), rowvar=False),
            'Utilities': np.cov(util_returns.to_numpy(), rowvar=False),
        }

        returns_by_block = {
            'Technology': tech_returns.to_numpy(),
            'Utilities': util_returns.to_numpy(),
        }

        # Calculate condition numbers before shrinkage
        cond_before = {
            sector: np.linalg.cond(cov)
            for sector, cov in blocks.items()
        }

        # Apply per-block shrinkage
        estimator = LedoitWolfShrinkage()
        shrunk_blocks = estimator.apply_per_block_shrinkage(
            blocks, returns_by_block
        )

        # Get shrinkage intensities to understand what's happening
        alphas = estimator.get_block_shrinkage_intensities()

        # Calculate condition numbers after shrinkage
        cond_after = {
            sector: np.linalg.cond(cov)
            for sector, cov in shrunk_blocks.items()
        }

        # Condition numbers should be reduced if shrinkage is applied
        # If alpha is very small, shrinkage may not help much
        for sector in blocks.keys():
            if alphas[sector] > 0.01:  # Only check if meaningful shrinkage
                assert cond_after[sector] <= cond_before[sector]
            # Should meet business requirement
            assert cond_after[sector] < 500  # Relaxed requirement for test data

    def test_different_alpha_per_block(self):
        """Each block should have its own shrinkage intensity α."""
        from Risk.Covariance.LedoitWolfShrinkage import LedoitWolfShrinkage

        np.random.seed(42)

        # Block 1: Well-conditioned (T >> N, should have low α)
        block1_returns = pl.DataFrame(
            np.random.randn(100, 5),
            schema=['A1', 'A2', 'A3', 'A4', 'A5']
        )

        # Block 2: Ill-conditioned (T ≈ N, should have high α)
        block2_returns = pl.DataFrame(
            np.random.randn(30, 25),
            schema=[f'B{i}' for i in range(25)]
        )

        # Create blocks (use numpy.cov for proper covariance computation)
        blocks = {
            'WellConditioned': np.cov(block1_returns.to_numpy(), rowvar=False),
            'IllConditioned': np.cov(block2_returns.to_numpy(), rowvar=False),
        }

        returns_by_block = {
            'WellConditioned': block1_returns.to_numpy(),
            'IllConditioned': block2_returns.to_numpy(),
        }
        ...
        assert len(alphas) == 2
        assert 'WellConditioned' in alphas
        assert 'IllConditioned' in alphas

        # Ill-conditioned block should have higher shrinkage
        assert alphas['IllConditioned'] > alphas['WellConditioned']

        # Both should be in [0, 1]
        for alpha in alphas.values():
            assert 0 <= alpha <= 1

    def test_preserves_block_sizes(self):
        """Shrinkage should preserve the size of each block."""
        from Risk.Covariance.LedoitWolfShrinkage import LedoitWolfShrinkage

        np.random.seed(42)

        # Create blocks of different sizes
        block1_returns = pl.DataFrame(np.random.randn(60, 3))
        block2_returns = pl.DataFrame(np.random.randn(60, 7))
        block3_returns = pl.DataFrame(np.random.randn(60, 5))

        blocks = {
            'Small': np.cov(block1_returns.to_numpy(), rowvar=False),
            'Large': np.cov(block2_returns.to_numpy(), rowvar=False),
            'Medium': np.cov(block3_returns.to_numpy(), rowvar=False),
        }

        returns_by_block = {
            'Small': block1_returns.to_numpy(),
            'Large': block2_returns.to_numpy(),
            'Medium': block3_returns.to_numpy(),
        }
        ...
        assert shrunk_blocks['Small'].shape == (3, 3)
        assert shrunk_blocks['Large'].shape == (7, 7)
