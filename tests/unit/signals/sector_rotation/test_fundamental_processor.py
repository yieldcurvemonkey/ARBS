...
class TestFundamentalProcessor:
    ...
    def test_detect_outliers_z_score(self):
        """Test outlier detection using z-score > 3."""
        from Signals.SectorRotation.FundamentalProcessor import FundamentalProcessor
        ...
        fundamental_df = pl.DataFrame({
            "ticker": ["XLK", "XLE", "XLF", "XLI", "XLV"],
            "date": [date(2023, 3, 31)] * 5,
            "pe_ratio": [25.0, 15.0, 12.0, 18.0, 150.0],  # 150 is outlier
            "pb_ratio": [3.5, 1.2, 1.0, 2.0, 2.5],
            "ev_sales": [4.0, 2.0, 2.5, 3.0, 3.5],
            "ev_ebit": [18.0, 8.0, 10.0, 12.0, 14.0],
            "ev_ebitda": [15.0, 6.0, 8.0, 10.0, 12.0],
            "dividend_yield": [0.01, 0.04, 0.03, 0.02, 0.02],
            "gross_margin": [0.45, 0.20, 0.35, 0.30, 0.40],
            "operating_margin": [0.25, 0.10, 0.20, 0.15, 0.22],
            "profit_margin": [0.18, 0.08, 0.15, 0.12, 0.16],
            "roa": [0.12, 0.05, 0.08, 0.10, 0.11],
            "roe": [0.25, 0.10, 0.15, 0.18, 0.20],
        })

        outlier_info = processor.detect_outliers(fundamental_df)

        # Should detect XLV's pe_ratio as outlier (z > 3)
        assert len(outlier_info) > 0
        assert any("pe_ratio" in str(info) and "XLV" in str(info) for info in outlier_info)

    def test_winsorize_outliers(self):
        """Test outlier winsorization (cap at ±3 std)."""
        from Signals.SectorRotation.FundamentalProcessor import FundamentalProcessor
        ...
        fundamental_df = pl.DataFrame({
            "ticker": ["XLK", "XLE", "XLF", "XLI", "XLV"],
            "date": [date(2023, 3, 31)] * 5,
            "pe_ratio": [25.0, 15.0, 12.0, 18.0, 150.0],  # 150 is extreme
            "pb_ratio": [3.5, 1.2, 1.0, 2.0, 2.5],
            "ev_sales": [4.0, 2.0, 2.5, 3.0, 3.5],
            "ev_ebit": [18.0, 8.0, 10.0, 12.0, 14.0],
            "ev_ebitda": [15.0, 6.0, 8.0, 10.0, 12.0],
        ...
        winsorized_df = processor.winsorize_outliers(fundamental_df)

        # XLV's pe_ratio should be capped (not 150 anymore)
        xlv_pe = winsorized_df.filter(pl.col("ticker") == "XLV")["pe_ratio"][0]
        assert xlv_pe < 150.0  # Should be winsorized
        assert xlv_pe > 0  # Should still be positive
    ...
    def test_process_pipeline(self):
        ...
        dates = [date(2023, 3, 31), date(2023, 6, 30)]
        fundamental_df = pl.DataFrame({
            "ticker": ["XLK"] * 2 + ["XLE"] * 2,
            "date": dates * 2,
            "pe_ratio": [25.0, None, 15.0, 200.0],  # Missing + outlier
            "pb_ratio": [3.5, 3.6, 1.2, 1.3],
            "ev_sales": [4.0, 4.1, 2.0, 2.1],
            "ev_ebit": [18.0, 18.5, 8.0, 8.5],
            "ev_ebitda": [15.0, 15.5, 6.0, 6.5],
            "dividend_yield": [0.01, 0.01, 0.04, 0.04],
            "gross_margin": [0.45, 0.46, 0.20, 0.21],
            "operating_margin": [0.25, 0.26, 0.10, 0.11],
            "profit_margin": [0.18, 0.19, 0.08, 0.09],
            "roa": [0.12, 0.13, 0.05, 0.06],
            "roe": [0.25, 0.26, 0.10, 0.11],
        })
        ...
        processor.validate_schema(processed_df)

        # Should have no None values (forward-filled)
        for col in processor.factor_columns:
            assert processed_df[col].null_count() == 0

        # Outliers should be handled
        max_pe = processed_df["pe_ratio"].max()
        assert max_pe < 200.0  # Outlier winsorized
...
if __name__ == "__main__":
    ...
    test.test_detect_outliers_z_score()
    print("✓ test_detect_outliers_z_score")

    test.test_winsorize_outliers()
    print("✓ test_winsorize_outliers")

    test.test_process_pipeline()
    print("✓ test_process_pipeline")
