...
def test_match_unwinds_direct_lineage():
    # Legal-delay classes (Task A1): unwind rows only carry is_block, no
    # on_facility/cleared/is_capped, so on_facility is always indeterminate
    # here -> always +60min regardless of is_block. See unwinds.py comment.
    ...
    assert out.iloc[0]["unwind_visibility_ts"] == pd.Timestamp("2026-07-11 15:00:00+00:00")
