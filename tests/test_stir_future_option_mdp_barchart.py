...
def test_live_snapshot_atmf_offset_aliases(monkeypatch):
    ...
    class _DummyBC:
        def get_option_quotes(self, symbols, **kwargs):
            ...
            call_df = pd.DataFrame(
                [
                    {"strikePrice": 96.00, "bidPrice": 0.18, "askPrice": 0.20, "lastPrice": 0.19, "tradeTime": 1760000000},
                    {"strikePrice": 96.25, "bidPrice": 0.15, "askPrice": 0.17, "lastPrice": 0.16, "tradeTime": 1760000000},
                    {"strikePrice": 96.50, "bidPrice": 0.11, "askPrice": 0.13, "lastPrice": 0.12, "tradeTime": 1760000000},
                ]
            )
            put_df = pd.DataFrame(
                [
                    {"strikePrice": 96.00, "bidPrice": 0.07, "askPrice": 0.09, "lastPrice": 0.08, "tradeTime": 1760000000},
                    {"strikePrice": 96.25, "bidPrice": 0.10, "askPrice": 0.12, "lastPrice": 0.11, "tradeTime": 1760000000},
                    {"strikePrice": 96.50, "bidPrice": 0.16, "askPrice": 0.18, "lastPrice": 0.17, "tradeTime": 1760000000},
                ]
            )
            return {"SQZ30": {"call": call_df, "put": put_df}}
        ...
        def barchart_timeseries_api(self, **kwargs):
            assert kwargs["interval"] == 1
            idx = pd.DatetimeIndex([pytz.timezone("America/Chicago").localize(datetime.datetime(2026, 1, 2, 10, 30))])
            return {"SQZ30": pd.DataFrame({"Open": [96.20], "High": [96.24], "Low": [96.19], "Close": [96.23]}, index=idx)}
...
def test_fetch_bulk_sabr_smile_barchart_offset_mode_reuses_shared_window(monkeypatch):
    ...
    z_atm_call = f"SFRZ26|{stirfo_module._format_strike4(95.75, contract='SFRZ26')}C"
    z_call_1250 = f"SFRZ26|{stirfo_module._format_strike4(95.625, contract='SFRZ26')}C"
    z_call_2500 = f"SFRZ26|{stirfo_module._format_strike4(95.5, contract='SFRZ26')}C"
    z_atm_put = f"SFRZ26|{stirfo_module._format_strike4(95.75, contract='SFRZ26')}P"
    z_put_1250 = f"SFRZ26|{stirfo_module._format_strike4(95.875, contract='SFRZ26')}P"
    z_put_2500 = f"SFRZ26|{stirfo_module._format_strike4(96.0, contract='SFRZ26')}P"
    ...
    def _stub_window(**kwargs):
        ...
        return {
            ...
            z_atm_call: {
                d1: _mk_option_pricer(symbol=z_atm_call, quote_timestamp=ts1, expiry_date=expiry, iv_normal=0.165, forward=95.74, delta=0.50, underlying_symbol="SFRZ26"),
                d2: _mk_option_pricer(symbol=z_atm_call, quote_timestamp=ts2, expiry_date=expiry, iv_normal=0.166, forward=95.70, delta=0.50, underlying_symbol="SFRZ26"),
            },
            z_call_1250: {
                d1: _mk_option_pricer(symbol=z_call_1250, quote_timestamp=ts1, expiry_date=expiry, iv_normal=0.172, forward=95.74, delta=0.30, underlying_symbol="SFRZ26"),
                d2: _mk_option_pricer(symbol=z_call_1250, quote_timestamp=ts2, expiry_date=expiry, iv_normal=0.173, forward=95.70, delta=0.30, underlying_symbol="SFRZ26"),
            },
            z_call_2500: {
                d1: _mk_option_pricer(symbol=z_call_2500, quote_timestamp=ts1, expiry_date=expiry, iv_normal=0.179, forward=95.74, delta=0.18, underlying_symbol="SFRZ26"),
                d2: _mk_option_pricer(symbol=z_call_2500, quote_timestamp=ts2, expiry_date=expiry, iv_normal=0.180, forward=95.70, delta=0.18, underlying_symbol="SFRZ26"),
            },
            z_atm_put: {
                d1: _mk_option_pricer(symbol=z_atm_put, quote_timestamp=ts1, expiry_date=expiry, iv_normal=0.165, forward=95.74, delta=-0.50, underlying_symbol="SFRZ26"),
                d2: _mk_option_pricer(symbol=z_atm_put, quote_timestamp=ts2, expiry_date=expiry, iv_normal=0.166, forward=95.70, delta=-0.50, underlying_symbol="SFRZ26"),
            },
            z_put_1250: {
                d1: _mk_option_pricer(symbol=z_put_1250, quote_timestamp=ts1, expiry_date=expiry, iv_normal=0.171, forward=95.74, delta=-0.30, underlying_symbol="SFRZ26"),
                d2: _mk_option_pricer(symbol=z_put_1250, quote_timestamp=ts2, expiry_date=expiry, iv_normal=0.172, forward=95.70, delta=-0.30, underlying_symbol="SFRZ26"),
            },
            z_put_2500: {
                d1: _mk_option_pricer(symbol=z_put_2500, quote_timestamp=ts1, expiry_date=expiry, iv_normal=0.178, forward=95.74, delta=-0.18, underlying_symbol="SFRZ26"),
                d2: _mk_option_pricer(symbol=z_put_2500, quote_timestamp=ts2, expiry_date=expiry, iv_normal=0.179, forward=95.70, delta=-0.18, underlying_symbol="SFRZ26"),
            },
        }
