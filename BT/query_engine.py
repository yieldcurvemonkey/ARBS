...
class PositionHandler:
    ...
    def value_position(
        ...
        pricer_or_curve: _GenericPricer | Dict[str, _GenericPricer] = pricer_provider(
...
class FinancedFixedRateBondHandler(PositionHandler):
    ...
    def _maybe_roll_position(
        ...
        *,
        ...
        trigger = ConstantMaturityRollTrigger(
            ConstantMaturityRollTriggerRequirements(timestamp=now, previous=resolved_prev, new=resolved_now)
        )
    ...
    def on_mark(
        ...
        pos1, cf_realized = self._apply_cashflows(
            position, pricer_provider=pricer_provider, now=now, backtest=backtest
        )
        pos2, roll_realized, triggers = self._maybe_roll_position(
            pos1, pricer_provider=pricer_provider, now=now, backtest=backtest
        )
...
class QueryDrivenBacktest:
    ...
    def _normalize_query_for_resolution(self, q: BaseQuery) -> BaseQuery:
        ...
        if slash_count == 1:
            ...
            if all(
                skw.get(k) is None for k in ("front_notional", "belly_notional", "back_notional", "bpv")
            ):
                skw["bpv"] = 1.0
    ...
    def _handle_unwind(self, order: UnwindOrder, now: datetime.datetime) -> None:
        ...
        for pos in to_close:
            handler = self._handler_for_position(pos)
            realized, triggers = handler.on_unwind(
                pos, lambda q: self._pricer_for_query(q, now), now, self
            )
            pnl += float(realized)
            if triggers:
    ...
    def mark_to_market(self, now: datetime.datetime) -> float:
        ...
        for p in self.portfolio.iter_positions():
            ...
            p1, realized_delta, triggers = handler.on_mark(
                p, lambda q: self._pricer_for_query(q, now), now, self
            )
